...
def find_date(txt: str, original_update_time_value=None, reg: str = "Entered on -", window: int = 20, verbosity: int = 0) -> List[Dict[str, Any]]:
    ...
    for match in m:
        ...
        chunks.append({'text': chunk_t, 'date_text': dw, 'date': date,
                      'date_found': date_found, 'text_start': text_start, 'text_end': text_end})
...
def split_clinical_notes(clin_note: pd.DataFrame, verbosity_val=0) -> pd.DataFrame:
    ...
    for ex in extracted:
        counter = 0
        for ch in ex['chunks']:
            nd = {'client_idcode': ex['client_idcode'],
                  'body_analysed': ch['text'], 'updatetime': ch['date']}
...
def split_clinical_notes_mct(clin_note: pd.DataFrame, verbosity_val=0) -> pd.DataFrame:
    ...
    for index, row in clin_note.iterrows():
        ...
        try:
            ...
            document_description_list.append(
                row['obscatalogmasteritem_displayname'])
            id_list.append(row['id'])
    ...
    for ex in extracted:
        counter = 0
        for ch in ex['chunks']:
            nd = {'client_idcode': ex['client_idcode'], 'observation_valuetext_analysed': ch['text'],
...
def split_and_append_chunks(docs: pd.DataFrame, epr=True, mct=False, verbosity: int = 0) -> pd.DataFrame:
    ...
    if column_name in docs.columns:
        ...
        clinical_notes = docs[docs[column_name_mct]
                              == 'AoMRC_ClinicalSummary_FT']
        non_clinical_notes = docs[docs[column_name_mct]
                                  != 'AoMRC_ClinicalSummary_FT']
        if verbosity > 1:
    ...
    if verbosity > 1:
        ...
        print(
            f"Size of non_clinical_notes dataframe: {len(non_clinical_notes)}")
    ...
    concatenated_notes = pd.concat(
        [non_clinical_notes, split_clinical_notes_result])

    concatenated_notes = pd.concat(
        [concatenated_notes, none_found], ignore_index=True)
